...
struct CatalogTable
{
	...
	unsigned short CatalogTable_FirstKey;	// key 目录文件中，该表第一条键信息的编号
...
public :
	...
	//功能需求
	void CatalogCreateTable(string & Tablename, vector<CatalogAttributes> & catalogattributes);	//建立数据表的模式信息文件
	void CatalogCheckTable(string &Tablename, vector<CatalogAttributes> &	catalogattributes);		//数据表的检查
	...
	*/
	vector<Table> TableCatalog;														//数据表存放容器
	vector<Attributes>AttributesCatalog;											//属性的存放
	/*
		获取操作
	*/
