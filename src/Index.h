...
public:
	...
	template<typename KEY> void Index_TreeInsert(BPlusTree<KEY> &tree ,vector<pair<string,int>> & Info);
};
...
void Index_Manager::Index_TreeInsert(BPlusTree<KEY> &tree, vector<pair<string, int>> & Info)
{
	for (auto i = Info.begin(); i != Info.end(); i++)
		tree.BPlusTree_Insert(i->first, i->second);
}
#endif